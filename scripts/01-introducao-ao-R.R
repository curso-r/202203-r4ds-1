...
# podemos fazer anotações, e explições sobre o nosso código
...
nrow(mtcars) # mtcars é um argumento neste caso!
ncol(mtcars)

# exemplo de funcao com 2 argumentos
round(1.76432, 1) # arredondando

ceiling(1.76432) # arredonda para o numero inteiro acima
floor(1.76432) # arredonda para o numero inteiro para baixo
...
read.csv("dados/imdb.csv") # é melhor fazer assim!

# mas também é possível buscar em outras pastas
# e também na internet:

read.csv("~/Downloads/imdb.csv")
#https://github.com/curso-r/main-r4ds-1/raw/master/dados/imdb.csv
...
1x <- 1
_objeto <- 2
meu-objeto <- 3 # - é usado para fazer subtração.
...
# tentem usar o tab entre aspas para procurar o nome do objeto!
...
# CRIADOS COM A FUNCAO c() - de combinar!
...
titulos_dos_filmes <- imdb$titulo

ano_dos_filmes <- imdb$ano
anos_unicos_de_filmes <- unique(ano_dos_filmes)
length(anos_unicos_de_filmes) # length permite ver o tamanho do vetor!
...
imdb$titulo[1:6] # busca os valores pela posicao/ordem
head(imdb$titulo)
tail(imdb$titulo)
