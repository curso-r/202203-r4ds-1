...
plot_mananciais <- mananciais %>%
  ...
  facet_wrap( ~ sistema,
              labeller = as_labeller(function(nome_sistema) paste('Sistema',
                                                                  nome_sistema)))

plot_mananciais


colar_nome_sistema <- function(x){
  paste("Sistema", x)
}

#colar_nome_sistema("Cantareira")
...
imdb %>%
  ...
  geom_point(aes(x = orcamento, y = receita)) +
  gghighlight(
    receita > 2000000000, # uma comparação lógica, que gera
    # TRUE/FALSE, para usar para destacar os pontos.
    label_key = titulo # qual é a coluna que queremos usar
    # para colocar uma anotação??
  )
